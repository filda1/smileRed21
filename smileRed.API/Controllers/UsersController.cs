    ...
    public class UsersController : ApiController
    {
        ...
        public async Task<IHttpActionResult> PasswordRecovery(JObject form)
        {
            try
            {
              ...
              UsersHelper.CreateUserASP(user.Email, "User", user.Password);
                var userASP2 = userManager.FindByEmail(user.Email);

                if (userASP2 != null)
                {
                    var subject = "App - Password Recovery";
                    var body = string.Format(@"
                        <h1>App - Password Recovery</h1>
        ...
        /* [HttpPost]
         ...
         public IHttpActionResult LoginFacebook(FacebookResponse profile)
         {
             ...
             var answer = UsersHelper.CreateUserASP_Obj(userRequest);
             return Ok(answer);
         }
         ...
         public async Task<IHttpActionResult> PutUser(int id, User user)
         {
             if (user.ImageArray != null && user.ImageArray.Length > 0)
             {
                 ...
                 if (response)
                 {
                     // fullPath = ~/Content/Images/xxxxxxxxx.jpg

                     string[] breakp = fullPath.Split('/');
                     string urlImage = "http://aurora.somee.com/" + breakp[1] + "/" + breakp[2] 
                                     + "/" + breakp[3];
        ...
        // http://...../api/Users/PutUserID/21
        [HttpPut]
        [Authorize]     
        [Route("PutUserID/{id}")]
        public async Task<IHttpActionResult> PutUserID(int id, User user)
        {
        ...
        public async Task<IHttpActionResult> PostUser(User user)
        {
            if (user.ImageArray != null && user.ImageArray.Length > 0)
            {
                ...
                var guid = Guid.NewGuid().ToString();
                var file = string.Format("{0}.jpg", guid);
                var folder1 = "~/Content/Images";
                var fullPath = string.Format("{0}/{1}", folder1, file);
                var response = FilesHelper.UploadPhoto(stream, folder1, file);
        ...
        [Authorize]
        [Route("GetUserAsp")]
        public IHttpActionResult GetUserAsp(JObject form)
        {
            try
            {
                var email = string.Empty;
